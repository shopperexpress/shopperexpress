...
if ( is_single($post_id) || get_post_type($post_id) == 'append-data' || get_post_type($post_id) == 'offers' ) {
    $field = !in_array($post_type, [ 'listings', 'used-listings' ]) ? $post_type . '-detail_info_detail_info' : 'detail_info';
}else{
    ...
    <dl class="<?php echo $class; ?>">
        ...
        while ( have_rows( $field, 'options' ) ) : the_row();
            ...
            if ( $value = get_sub_field( 'value' ) ) :
                $value = explode( ' ', $value );
                ?>
                <dd<?php if ( str_contains(strtolower($label), 'vin' )) : ?> class="vin"<?php endif; ?>>
                    <?php
                    $terms = [];
