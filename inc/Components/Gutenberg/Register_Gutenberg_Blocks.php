...
class Register_Gutenberg_Blocks implements Theme_Component {
	...
	private function get_blocks(): array {
		...
		return ( $acf_gutenberg_blocks ) ? $acf_gutenberg_blocks : array();
	}
	...
	private function get_block_data( string $path, string $name ): array {
		...
		$block_data['keywords']    = ( $block_data['keywords'] ) ? array_map( 'trim', explode( ', ', $block_data['keywords'] ) ) : array( $block_data['title'], $block_data['description'] );
