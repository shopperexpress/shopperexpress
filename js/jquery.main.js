...
function initSlickCarousel() {
	...
	// var activeClass = 'no-transform';

	// jQuery('.detail-slider-nav').each(function() {
...
function initFiltering() {
	jQuery('.filter-section').filteringItems({
		items: '.slide',
	...
	AutoCompleteForm.prototype = {
		...
		showDrop: function() {
			if (this.input.val().length >= this.options.startCount) {
				...
				// if(!this.listItems.filter(':visible').length) this.hideDrop();
			} else {
				this.resultsBox.hide();
				this.isDropShow = false;
