...
;(function($) {
	...
	AjaxFiltering.prototype = {
		...
		findElements: function() {
			...
			this.rangeFields = this.filterBlock.find('[type="range"]');
			this.switchers = this.filterBlock.find('.filter-switcher [type="checkbox"]');
			...
			this.filterModal = $('.modal-filter');
			this.modalBtnClear = this.filterModal.find(this.options.btnReset);
			this.btnReset = this.holder.find('.btn-reset');
			this.modalFilterForm = $('.modal-filter-form');
			this.checkboxes = this.filterModal.find(':checkbox');
			this.ajaxBusy = false;
			this.filtersArr = [];
			this.isChangePrice = false;
			this.currFilterGroup = null;
		},
		attachEvents: function() {
			var self = this;

			this.holder.on('submit', function(e) {
				e.preventDefault($(e.target));
			});

			this.rangeFields.on('change', function(e) {
				var field = $(this);
			...
			this.searchField.on('search', function(e) {
				if (self.searchField.val().trim() !== '') {
					self.sendRequest($(e.target));
					self.searchField.val('').blur();
				}
			});
			...
			this.checkboxes.on('change', function(e) {
				...
				self.currFilterGroup = $(this).attr('name');
			});
			...
			this.selectedFiltersHolder.on('click', '.remove', function(e) {
				...
				var parent = btnRemove.closest('li');
			...
			this.switchers.on('change', function() {
				self.switchers.not(this).each(function() {
					var currSwitcher = $(this);

					currSwitcher.removeAttr('checked');
					currSwitcher.prop('checked', false);

					setTimeout(function() {
						currSwitcher.closest('label').removeClass('input-checked-parent input-checked');
					}, 10);
				});
			...
			this.modalBtnClear.on('click', function(e) {
				e.preventDefault();
				self.resetForm();
			});
			...
			this.sendRequest = function(item) {
				if (!self.ajaxBusy) {
					self.container.empty();

					if (item && window.history && window.history.pushState) {
						if (item.is(':checkbox')) {
							if (item.is(':checked')) {
								window.StorageHistory.set(item.attr('name'), item.val(), true);
							} else {
								window.StorageHistory.remove(item.attr('name'), item.val());
							}
						} else {
							window.StorageHistory.set(item.attr('name'), item.val());
						}
					} else {
			...
			this.scrollHandler = function() {
				if (!self.btnMore.length || self.ajaxBusy) return;
		...
		ajaxRequest: function(src) {
			var self = this;
			var serialize = this.holder.serialize();
			var search = getUrlAttr('search');
			var filtersObj = {};
			var filterSerialize = '';

			if (search !== undefined) {
				if (serialize.indexOf('search=' + search) === -1) {
					var newStr = 'search=' + search;

					serialize = serialize.replace('search=', newStr);
				}
			}

			this.ajaxBusy = true;
			this.holder.add(this.filterModal).addClass(this.options.loadClass);
			...
			this.checkboxes.each(function(e) {
				...
				if (item.is(':checked')) {
					...
					filtersObj[category].push(item.val());
				}
			});

			for (const key in filtersObj) {
				filterSerialize += '&' + key + '=' + filtersObj[key].join(',');
			}
		...
		onSuccess: function(data) {
			...
			if (btnMore.length) {
				this.btnMore.attr('href', btnMore.attr('href')).show();
			} else {
				this.btnMore.attr('href', '#').hide();
			}

			setTimeout(function() {
				items.appendTo(self.container).css('opacity', 0);

				items.stop().animate({
					opacity: 1
				}, self.options.delay);

				self.holder.add(self.filterModal).removeClass(self.options.loadClass);
				self.ajaxBusy = false;
				self.makeCallback('ajaxComplete', items, loadedBlock);
			}, this.options.delay);
		},
		makeCallback: function(name) {
			if (typeof this.options[name] === 'function') {
...
;(function($) {
	...
	window.StorageHistory = {
		set: function(name, value, group) {
			...
			window.history.pushState(stateObject, name, '?' + tempValues);
		},
		...
		reset: function() {
			...
			window.history.replaceState(stateObject, null, url);
		}
	};
...
;(function($) {
	...
	AutoCompleteForm.prototype = {
				...
				if (self.listItems) {
					...
					switch (e.keyCode) {
						case 13:
							...
							if (self.isDropShow && self.currentIndex !== -1) {
		...
		updateDrop: function(text) {
			...
			if (self.lastData !== text) {
				var output;

				self.lastData = text;
				self.currentIndex = -1;

				if (self.options.dataType.toLowerCase() === 'json' && typeof self.options.parseData === 'function') {
					output = self.options.parseData.call(self, text) || text;
				} else {
					output = text;
				}
				...
				self.listItems.each(function() {
					...
					curItem.click(function() {
						if (self.options.redirectOnLink) {
							var link = curItem.find('a');
	...
	jQuery.fn.autoCompleteForm = function(options) {
		return this.each(function() {
			jQuery(this).data('AutoCompleteForm', new AutoCompleteForm(jQuery.extend(options, {
				holder: this
			})));
		});
	};
...
function initRemoveBlock() {
	jQuery('#block-1 .payment-info').each(function() {
		...
		items.each(function(i) {
			var item = jQuery(this);
			var textHolder = item.find('.text-holder');
			var siblings = textHolder.siblings();
...
function initFieldsSwitcher() {
	var holder = jQuery('.form-offer');

	holder.each(function() {
		var checkedClass = 'input-checked';
		var buttons = holder.find('[data-name]');
		var fields = holder.find('[data-field]');
...
function initStickyClass() {
	...
	var searchPanel = page.find('.sticky-wrap-sticky-panel');
	var activeClass = 'sticky-search';
...
function initOfferForm() {
	jQuery('.form-offer').each(function() {
		...
		var injectPlateField = form.find('.inject-plate');
		var injectVinField = form.find('.inject-vin');
		var injectStateField = form.find('.inject-state');
		var injectMilesField = form.find('.inject-miles');
		var allFields = form.find('.form-control');
		var injectPlate, injectVin, injectState, injectMiles;
...
function initRegistration() {
	...
	jQuery('.form-unlock').each(function() {
		var form = jQuery(this);
		var btnRegister = form.find('#register-button');
		var message = form.find('.register-message');

		btnRegister.on('click', function(e) {
			e.preventDefault();

			var data = form.serialize() + '&action=register_user';
...
function initFilteringModal() {
	...
	var filterModal = jQuery('.modal-filter');
	var btnClose = filterModal.find('[data-dismiss="modal"]:not(.btn-back)');
	var activeClass = 'filter-modal-active';
...
function initCustomForms() {
	...
	jQuery('.range-row').each(function() {
		var holder = jQuery(this);
		var field = holder.find('[type="range"]');
		var minPrice = holder.find('.min-price');
		var maxPrice = holder.find('.max-price');
		var hiddenField = holder.find('[type="hidden"]');

		field.on('input', function() {
			var values = field.data('jcfInstance').values;

			minPrice.text(formatToMoney((values[0]).toString().trim()));
			maxPrice.text(formatToMoney((values[1]).toString().trim()));
	...
	jQuery('.se-sbp-widget__range').each(function() {
		var holder = jQuery(this);
		var field = holder.find('[type="range"]');
		var rangeLine = holder.find('.jcf-range-display');
		var valueField = holder.find('.se-sbp-widget__range-input');
		var currency = field.data('currency');
		var rangeValue = field.data('range-value');
		var max = field.attr('max');
		var activeClass = 'half-path';
...
function initShopButton() {
	jQuery('.se-sbp-widget').each(function() {
		var holder = jQuery(this);
		...
		function filterItems() {
			...
			checkItems.each(function() {
				...
				if (item.is(':checked')) {
					checkedArr.push(label.text().toLowerCase().trim());
				}
			});
...
function initAjaxFiltering() {
	jQuery('.filter-section').ajaxFiltering({
		...
		onInit: function(self) {
			var dataItem = this.holder.find('.json-data');

			this.filterCheckboxes = jQuery('.modal-filter [type="checkbox"]');
			this.totalCount = this.holder.find('.result-total').add(jQuery('.total-counts'));
			...
			this.updateCounters = function(dataItem) {
				if (dataItem.length) {
					...
					if (data) {
						...
						self.filterCheckboxes.each(function() {
							var item = jQuery(this);
							var parent = item.closest('li');
							var name = item.attr('name');
							var value = item.val();
							var countItem = parent.find('.detail-count');
							var filteredArray = [];

							if (self.currFilterGroup === name) return;

							if (name === 'yr') {
								name = 'year';
							}

							if (data.taxonomies[name]) {
								filteredArray = data.taxonomies[name].filter(function(el) {
									return el.name === value;
								});
							}

							for (const key in filteredArray[0]) {
								if (key === 'count') {
									countItem.text(filteredArray[0][key]);

									if (filteredArray[0][key] === 0) {
										if (!item.is(':checked')) {
											parent.hide();
										}
									} else {
					...
					if (visibleItems.length > 1) {
						self.visibleCount.text('1-' + visibleItems.length);
					} else {
						self.visibleCount.text('0');
					}
				}

				setTimeout(function() {
					if (self.filterCheckboxes.filter(':checked').length) {
						self.modalBtnClear.removeAttr('disabled');
					} else {
						self.modalBtnClear.attr('disabled', true);
					}
				}, 100);
			};
		...
		ajaxComplete: function(items, data) {
			var dataItem = data.find('.json-data');
