<?php
/**
 * API-mode Conditional Incentive Offers widget (VDP).
 *
 * Reads pre-fetched My DealMaker offers from the Nexus vehicle payload
 * (`payload.incentive_offers`, populated server-side by Nexus's
 * FetchIncentiveOffersJob) instead of calling My DealMaker directly from the
 * theme — see template-parts/conditional-offers.php for the legacy (non-API,
 * ACF `listings` post) implementation, which is untouched and still makes its
 * own live My DealMaker request for sites not yet on Nexus API mode.
 *
 * Accepts $args:
...
<div class="conditional-offers">
	...
	<ul class="conditional-offers__list list-unstyled">
		<?php foreach ( $json as $index => $item ) : ?>
			<li>
				<a data-toggle="modal" href="#conditionalOffersDetail-<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $item['ProgramName'] ); ?>
					<svg
						aria-hidden="true"
...
add_action(
	'wp_footer',
	function () use ( $json ) {
		get_template_part( 'template-parts/conditionalOffersDetail', 'modal', array( 'json' => $json ) );
	}
);
