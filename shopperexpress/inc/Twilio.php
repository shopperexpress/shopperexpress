...
function send_sms_for_verification() {
	...
	if ( empty( $_POST['phone'] ) ) {
		wp_send_json_error( array( 'message' => 'Phone number not provided' ) );
	}
	...
	try {
		...
		wp_send_json_success( array( 'requestId' => $request_id ) );
	} catch ( Exception $e ) {
		error_log( 'Twilio Error: ' . $e->getMessage() );
		wp_send_json_error( array( 'message' => 'Twilio Error: ' . $e->getMessage() ) );
	}
}
