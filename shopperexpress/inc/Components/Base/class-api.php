			...
			if ( ! in_array( $post_id, $exclude ) ) {
				...
				switch ( $post_type ) {
					...
					case 'service-offers':
						...
						preg_match( '/\$(\d+(?:\.\d{2})?)/', get_field( 'offertext', $post_id ), $matches );
						$vehicle_data['price'] = ! empty( $matches[1] ) ? $matches[1] : 0;
					...
					case 'research':
						...
						$price                 = ( ! empty( $prices ) && ! empty( $prices[0]['price'] ) ) ? $prices[0]['price'] : 0;
						$vehicle_data['price'] = $price;
						if ( $imagelist = get_field( 'imagelistpath', $post_id ) ) {
							if ( is_array( $imagelist ) && ! empty( $imagelist[0]['image_url'] ) ) {
								$vehicle_data['photo'] = $imagelist[0]['image_url'];
							}
						}
