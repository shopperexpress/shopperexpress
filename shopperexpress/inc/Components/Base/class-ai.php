...
class AI implements Theme_Component {
	...
	public function faq_handler( $question ) {
		...
		$ai_message = $this->ask_ai( $prompt );

		if ( preg_match( '/\[(SALES|PARTS|SERVICE)\]/', $ai_message, $match ) ) {
			...
			$target = isset( $map[ $type ] ) ? $map[ $type ] : '#aiContactModal';

			$ai_message = preg_replace( '/\[(SALES|PARTS|SERVICE)\]/', '', $ai_message );

			$html  = '<div class="ai-chat__contact-btn-wrap">';
			$html .= '<button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="' . esc_attr( $target ) . '">' . esc_html__( 'Contact me', 'shopperexpress' ) . '</button>';
			$html .= '</div>';

			$ai_message .= "\n" . $html;
		}

		$result = array( 'message' => $ai_message );
