(function () {
	...
	function pushFormSubmissionEvents(formEl) {
		...
		var baseEvent  = Object.assign({}, payload, { event: 'asc_form_submission' });
		var typedEvent = Object.assign({}, payload, { event: 'asc_form_submission_' + formType });

		dl.events.push(baseEvent);
		dl.events.push(typedEvent);

		if (window.dataLayer && Array.isArray(window.dataLayer)) {
			window.dataLayer.push(baseEvent);
			window.dataLayer.push(typedEvent);
		}
	}
